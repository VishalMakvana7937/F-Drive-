...
let countCart= JSON.parse(localStorage.getItem('myItems'));
count.innerHTML = countCart.length;
...
const hendelclick = (id) =>{
    ...
    mycart.push(pc[0]);
    console.log("fiii",mycart.length);
    console.log(mycart);
...
p.forEach((l) => {
    ...
    c.innerHTML += `<div class="w-3 shop-c">
    <img src="${l.thumbnail}" alt="">
    <h2>${l.title}</h2>
    <h4>$ ${l.price}</h4>
    <button class="but" onclick="return hendelclick(${l.id})">ADD TO CART</button>
</div>`
})